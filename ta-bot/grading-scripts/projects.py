...
def json_add_testcases(project_repo: ProjectRepository = Provide[Container.project_repo]):
    ...
    try:
        ...
        for testcase in json_obj:
            project_repo.add_or_update_testcase(project_id, -1, testcase["levelname"], testcase["name"], testcase["description"], testcase["input"], testcase["output"], bool(testcase["isHidden"]), additional_file_path="")
        print("hi")
