...
GITHUB_OPSYS = "https://github.com/JForden/OpSys.git"
TEMP_PREFIX = "temp-"
...
def call_piston_api(student_file: str, testcase_in: str, language, additional_file_path: str):
        ...
        #print("This is the response", flush=True)
        #print(response, flush=True)
        if(response.ok):
            output_obj = response.json()
            #print(output_obj, flush=True)
            #print(output_obj["run"]["stdout"]) 
        return output_obj["run"]["stdout"]
...
def execute_test(filename, testcase_in, language, additional_file_path):
        ...
        if response == None:
            return "ERROR: API returned non-200 response"
        else:
            return response

def run_liter(myroot, output_dir, student_name, filename, language):
    if language == "python":
        data = subprocess.run(["pylint", os.path.join(myroot,filename), "--output-format=json"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = data.stdout
        ...
        file_path = os.path.join(myroot, filename)
        if not os.path.isdir(file_path):
            file = file_path
        else:
            files = [filename for filename in os.listdir(file_path) if filename.endswith('.java')]
            if "Main.java" in files:
                file = os.path.join(file_path, "Main.java")
            else:
        ...
        file_to_check = os.path.join(myroot, filename)
        file_to_check = os.path.join(myroot, filename)
...
def run(student_name, research_group, language, testcases, path, myroot):
    ...
    if os.path.isdir(path):
        ...
        filename = os.path.join(myroot,student_name+"."+extension)    
...
def main():
    ...
    parser.add_argument('project_name', metavar='project_name', type=str, help='name of the current project')
    ...
    if args.student_name != "ADMIN" and args.research_group != 1:
        testcases = json.loads(args.testcase_json)
        ...
        return CompilerRunner.run(args.student_name, args.language, args.testcase_json, args.paths, args.root, args.project_name)
    elif args.student_name == "ADMIN" and args.research_group == 1:
        return CompilerRunner.admin_run(args.testcase_json, args.paths, args.project_name)
    else:
         #args.testcase_json will just be a single value in the case of an admin upload. This value comes from the UI component
