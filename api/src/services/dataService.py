...
def run_local_plagiarism(
    ...
    # Prefer caller-provided language; otherwise read from project
    if not language:
        try:
            proj = project_repository.get_selected_project(int(projectid))
    ...
    result = detect_plagiarism(entries, language=language)
    return result
