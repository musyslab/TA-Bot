...
class CodeHelpComponent extends Component<CodeHelpComponentProps, CodeHelpComponentState, {}> {
    constructor(props: CodeHelpComponentProps) {
        ...
        axios.get(import.meta.env.VITE_API_URL + `/submissions/formcheck`, {
            headers: {
    ...
    handleFormSubmit() {
        ...
        axios.get(import.meta.env.VITE_API_URL + `/submissions/chatform?q1=${this.state.q1}&q2=${this.state.q2}&q3=${this.state.q3}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem("AUTOTA_AUTH_TOKEN")}`
            }
        })
    ...
    handleSubmit() {
        this.setState({ isLoading: true });
        axios.get(import.meta.env.VITE_API_URL + `/submissions/chatupload?code=${this.props.codedata}&question=${this.state.question}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem("AUTOTA_AUTH_TOKEN")}`
            }
        })
