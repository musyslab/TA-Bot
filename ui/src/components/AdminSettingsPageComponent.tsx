...
class AdminSettingsPageComponent extends Component<{}, AdminSettingsPageState> {
    ...
    componentDidMount() {
        axios.get(import.meta.env.VITE_API_URL + `/class/get_teacher_class_by_id`, {
            headers: {
    ...
    handleClick() {
        axios.post(import.meta.env.VITE_API_URL + `/settings/config`, { HasUnlockedEnabled: this.state.hasUnlockEnabled, HasScoreEnabled: this.state.hasScoreEnabled, HasTBSEnabled: this.state.hasTbsEnabled, HasLvlSysEnabled: this.state.hasLVLSYSEnabled, ClassId: this.state.classId },
            {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem("AUTOTA_AUTH_TOKEN")}`
                }
